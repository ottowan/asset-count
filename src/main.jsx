...
function App() {
  ...
  const [randomAuditFilter, setRandomAuditFilter] = useState('all');
  const [randomAuditType, setRandomAuditType] = useState('all');
  ...
  const sampledRandomAuditRows = randomAuditRows.filter((asset) => asset.round > 0);
  const typeFilteredRandomAuditRows = randomAuditType === 'all' ? sampledRandomAuditRows : sampledRandomAuditRows.filter((asset) => (asset.type || 'ไม่ระบุประเภท') === randomAuditType);
  const pendingRandomAuditRows = typeFilteredRandomAuditRows.filter((asset) => !counted[asset.id]);
  const filteredRandomAuditRows = randomAuditFilter === 'pending' ? pendingRandomAuditRows : typeFilteredRandomAuditRows;
  ...
  const openRandomAudit = () => {
    ...
    setRandomAuditFilter('all');
    setRandomAuditType('all');
  ...
  const exportRandomAudit = () => {
    ...
    XLSX.utils.book_append_sheet(workbook, sheet, randomAuditFilter === 'pending' ? 'รายการยังไม่นับ' : 'รายการสุ่มตรวจ');
    const filterSuffix = randomAuditFilter === 'pending' ? '-pending' : '-all';
    XLSX.writeFile(workbook, `random-audit${filterSuffix}-${new Date().toISOString().slice(0, 10)}-${projectFileName}.xlsx`);
  };
  ...
  return (
    <main>
      ...
      {showRandomAudit && <div className="random-audit-modal" role="dialog" aria-modal="true" aria-labelledby="random-audit-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowRandomAudit(false); }}><section className="random-audit-panel">
        ...
        <div className="random-type-plan"><button type="button" className={randomAuditType === 'all' ? 'active' : ''} onClick={() => setRandomAuditType('all')}><span>ทุกประเภท</span><strong>{sampledRandomAuditRows.length.toLocaleString('th-TH')}</strong><small>กดเพื่อดูรายการสุ่มทั้งหมด</small></button>{typeSamplingPlan.map((item) => { const sampledCount = sampledRandomAuditRows.filter((asset) => (asset.type || 'ไม่ระบุประเภท') === item.type).length; const remainingToSample = Math.max(item.available - sampledCount, 0); return <button type="button" className={randomAuditType === item.type ? 'active' : ''} onClick={() => setRandomAuditType(item.type)} key={item.type}><span>{item.type}</span><strong>{sampledCount.toLocaleString('th-TH')}</strong><small>สุ่มแล้ว {sampledCount.toLocaleString('th-TH')} · เป้าหมาย {item.target.toLocaleString('th-TH')} · เหลือ {remainingToSample.toLocaleString('th-TH')}</small></button>; })}</div>
        ...
        <div className="random-audit-actions"><div><button className="generate-random-button" type="submit" form="random-audit-form" disabled={isSavingRandomAudit || (!randomAvailableTarget && !done) || (randomAuditMode === 'rounds' && randomAvailableTarget > 0 && (!randomRoundTotal || randomRoundTotal > randomAvailableTarget))}>⌘ {isSavingRandomAudit ? 'กำลังบันทึก…' : randomAuditMode === 'all' ? 'สุ่มทั้งหมด' : 'สุ่มตามรอบ'}</button>{randomAuditRows.some((asset) => asset.round > 0) && <button className="clear-random-button" type="button" onClick={clearRandomAudit} disabled={isSavingRandomAudit}>ล้างเลขสุ่ม</button>}</div>{sampledRandomAuditRows.length > 0 && <button className="export-random-button" type="button" onClick={exportRandomAudit} disabled={!filteredRandomAuditRows.length}>⇩ Excel {randomAuditFilter === 'pending' ? 'ยังไม่นับ' : 'ทั้งหมด'} ({filteredRandomAuditRows.length.toLocaleString('th-TH')})</button>}</div>
        {sampledRandomAuditRows.length ? <><div className="random-audit-result-header"><p>{randomAuditType === 'all' ? 'รายการสุ่มใหม่ทุกประเภท' : `รายการสุ่ม ${randomAuditType}`} <strong>{filteredRandomAuditRows.length.toLocaleString('th-TH')}</strong> รายการ</p><div className="random-audit-filter"><button type="button" className={randomAuditFilter === 'all' ? 'active' : ''} onClick={() => setRandomAuditFilter('all')}>ทั้งหมด {typeFilteredRandomAuditRows.length.toLocaleString('th-TH')}</button><button type="button" className={randomAuditFilter === 'pending' ? 'active' : ''} onClick={() => setRandomAuditFilter('pending')}>ยังไม่นับ {pendingRandomAuditRows.length.toLocaleString('th-TH')}</button></div></div>{filteredRandomAuditRows.length ? <div className="random-audit-table-wrap"><table className="asset-table"><thead><tr><th>ลำดับ</th><th>รอบ</th><th>ประเภทอุปกรณ์</th><th>Pallet</th><th>Serial Number</th><th>สถานะ</th><th>สภาพ</th></tr></thead><tbody>{filteredRandomAuditRows.map((asset, index) => { const isCounted = Boolean(counted[asset.id]); const condition = countDetails[asset.id]?.condition; return <tr key={asset.id}><td>{index + 1}</td><td>{asset.round}</td><td><strong>{asset.type || '-'}</strong></td><td><strong>{asset.pallet || '-'}</strong></td><td><strong>{asset.sn}</strong><small>ID: {asset.id}</small></td><td><span className={`status-pill ${isCounted ? 'is-counted' : 'is-pending'}`}>{isCounted ? '✓ นับแล้ว' : '– ยังไม่นับ'}</span></td><td>{!isCounted ? '-' : condition === 'damaged' ? 'เสีย' : condition === 'good' ? 'ไม่เสีย' : 'ไม่ระบุ'}</td></tr>; })}</tbody></table></div> : <div className="random-audit-empty"><span>{typeFilteredRandomAuditRows.length ? '✓' : '⌕'}</span><p>{typeFilteredRandomAuditRows.length ? 'รายการที่สุ่มในประเภทนี้ถูกนับครบแล้ว' : 'ประเภทนี้ยังไม่มีรายการสุ่ม'}</p></div>}</> : <div className="random-audit-empty"><span>⌘</span><p>กำหนดรอบแล้วกดสุ่ม<br />หน้านี้จะแสดงเฉพาะรายการสุ่มใหม่</p></div>}
      </section></div>}
