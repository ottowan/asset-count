...
function App() {
  ...
  const pendingRandomAuditRows = sampledRandomAuditRows.filter((asset) => !counted[asset.id]);
  const filteredRandomAuditRows = randomAuditFilter === 'pending' ? pendingRandomAuditRows : sampledRandomAuditRows;
  ...
  return (
    <main>
      ...
      {showRandomAudit && <div className="random-audit-modal" role="dialog" aria-modal="true" aria-labelledby="random-audit-title" onMouseDown={(event) => { if (event.target === event.currentTarget) setShowRandomAudit(false); }}><section className="random-audit-panel">
        ...
        {sampledRandomAuditRows.length ? <><div className="random-audit-result-header"><p>รายการสุ่มใหม่ <strong>{filteredRandomAuditRows.length.toLocaleString('th-TH')}</strong> รายการ</p><div className="random-audit-filter"><button type="button" className={randomAuditFilter === 'all' ? 'active' : ''} onClick={() => setRandomAuditFilter('all')}>ทั้งหมด {sampledRandomAuditRows.length.toLocaleString('th-TH')}</button><button type="button" className={randomAuditFilter === 'pending' ? 'active' : ''} onClick={() => setRandomAuditFilter('pending')}>ยังไม่นับ {pendingRandomAuditRows.length.toLocaleString('th-TH')}</button></div></div>{filteredRandomAuditRows.length ? <div className="random-audit-table-wrap"><table className="asset-table"><thead><tr><th>ลำดับ</th><th>รอบ</th><th>ประเภทอุปกรณ์</th><th>Pallet</th><th>Serial Number</th><th>สถานะ</th><th>สภาพ</th></tr></thead><tbody>{filteredRandomAuditRows.map((asset, index) => { const isCounted = Boolean(counted[asset.id]); const condition = countDetails[asset.id]?.condition; return <tr key={asset.id}><td>{index + 1}</td><td>{asset.round}</td><td><strong>{asset.type || '-'}</strong></td><td><strong>{asset.pallet || '-'}</strong></td><td><strong>{asset.sn}</strong><small>ID: {asset.id}</small></td><td><span className={`status-pill ${isCounted ? 'is-counted' : 'is-pending'}`}>{isCounted ? '✓ นับแล้ว' : '– ยังไม่นับ'}</span></td><td>{!isCounted ? '-' : condition === 'damaged' ? 'เสีย' : condition === 'good' ? 'ไม่เสีย' : 'ไม่ระบุ'}</td></tr>; })}</tbody></table></div> : <div className="random-audit-empty"><span>✓</span><p>รายการที่สุ่มถูกนับครบแล้ว</p></div>}</> : <div className="random-audit-empty"><span>⌘</span><p>กำหนดรอบแล้วกดสุ่ม<br />หน้านี้จะแสดงเฉพาะรายการสุ่มใหม่</p></div>}
      </section></div>}
